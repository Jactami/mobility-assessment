...
ALTER TABLE
    ONLY public.pois
ADD
    CONSTRAINT pois_project_id_fkey FOREIGN KEY (project_id) REFERENCES public.projects(id) ON UPDATE CASCADE ON DELETE CASCADE;

-- Moddatetime extension
CREATE TRIGGER handle_updated_at_pois BEFORE
UPDATE
    ON public.pois FOR EACH ROW EXECUTE FUNCTION moddatetime('updated_at');