...
CREATE POLICY "Admins can read all pois." ON public.pois
  FOR SELECT TO authenticated
    USING (authorize('admin'));

CREATE POLICY "Admins can insert pois." ON public.pois
  FOR INSERT TO authenticated
    WITH CHECK (authorize('admin'));
...
CREATE POLICY "Admins can delete all pois." ON public.pois
  FOR DELETE TO authenticated
    USING (authorize('admin'));

CREATE POLICY "Authenticated users can read the pois of their own projects." ON public.pois
  FOR SELECT TO authenticated
    USING ((
      SELECT
        auth.uid()) =(
