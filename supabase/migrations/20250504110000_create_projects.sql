...
UPDATE
    ON public.projects FOR EACH ROW EXECUTE FUNCTION moddatetime('updated_at');

-- Set project limit
CREATE
OR REPLACE FUNCTION enforce_project_limit() RETURNS TRIGGER AS $ $ DECLARE existing_project projects % ROWTYPE;

BEGIN -- Check if project with the same ID already exists
SELECT
...
) >= 50 THEN RAISE EXCEPTION 'Project limit exceeded.' USING ERRCODE = 'P0001';

-- Custom user defined error code 
END IF;
...
CREATE TRIGGER check_project_limit BEFORE
INSERT
    ON public.projects FOR EACH ROW EXECUTE FUNCTION enforce_project_limit();